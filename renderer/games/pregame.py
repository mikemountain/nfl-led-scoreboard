...
def render_pregame(canvas, layout: Layout, pregame: Pregame):
    ...
    # OOP this
    graphics.DrawText(canvas, small_font, date_x, 32, graphics.Color(255, 255, 255), date_text)
    graphics.DrawText(canvas, small_font, time_x, 32, graphics.Color(255, 255, 255), time_text)
    graphics.DrawText(canvas, small_font, 25, 15, graphics.Color(255, 255, 255), 'VS')
    
    # OOP this
    away_team_logo = Image.open('assets/logos/{}.png'.format(pregame.away_team)).resize((20, 20), Image.BOX)
    home_team_logo = Image.open('assets/logos/{}.png'.format(pregame.home_team)).resize((20, 20), Image.BOX)
    # Put the images on the canvas
    canvas.SetImage(away_team_logo.convert("RGB"), 1, 12)
    canvas.SetImage(home_team_logo.convert("RGB"), 43, 12)
